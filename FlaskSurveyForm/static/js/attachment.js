...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addAttachmentPreview(attachment) {
    ...
    previewContainer.className =
      "attachment-preview d-flex align-items-center gap-3 mb-2 p-2 border rounded";
    ...
    filenameEl.textContent = attachment.isExisting
      ? attachment.filename
    ...
    removeButton.className =
      "btn btn-sm btn-outline-danger remove-attachment-btn";
  ...
  window.prepareAttachmentsForSubmit = function () {
    ...
    form
      .querySelectorAll(
        'input[name="attachments"], input[name="attachment_descriptions[]"], input[name="update_attachment_descriptions[]"]',
      )
      .forEach((input) => {
        input.remove();
      });
