...
function initEmployeeSearch(inputId, resultsContainer, onSelectCallback, options = {}) {
    console.log(`Initializing search for ${inputId}`);
    console.log(`Current employee list length: ${window.employeeList ? window.employeeList.length : 0}`);
    ...
    function performSearch(searchValue) {
        ...
        // Filter employees
        const matchingEmployees = window.employeeList.filter(employee => {
            return (employee.name && employee.name.toLowerCase().includes(searchValue)) || 
                   (employee.email && employee.email.toLowerCase().includes(searchValue)) ||
                   (employee.displayName && employee.displayName.toLowerCase().includes(searchValue));
        ...
        if (matchingEmployees.length > 0) {
            ...
            matchingEmployees.forEach(employee => {
                ...
                itemDiv.innerHTML = `
                    ...
                    ${employeeCopy.email ? `<div class="text-muted small">${employeeCopy.email}</div>` : ''}
                    ${employeeCopy.department ? `<div class="text-muted small fst-italic">${employeeCopy.department}</div>` : ''}
                `;
                ...
                itemDiv.addEventListener('click', function() {
                    ...
                    if (addToTrainees) {
                        addTrainee(employeeCopy.name, employeeCopy.email, employeeCopy.department);
                    }
        ...
        traineeItem.innerHTML = `
            <div class="card-body py-2">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-danger remove-trainee" data-email="${trainee.email}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const trainerSearchResults = document.getElementById('trainer-search-results');
    if (trainerSearchResults) {
        initEmployeeSearch('trainer_name_search', trainerSearchResults, function(employee) {
    ...
    const traineeSearchResults = document.getElementById('trainee-search-results');
    if (traineeSearchResults) {
        initEmployeeSearch('trainee-search-input', traineeSearchResults, function(employee) {
    ...
    if (addTraineeBtn) {
        addTraineeBtn.addEventListener('click', function() {
            const searchValue = document.getElementById('trainee-search-input').value.trim();
            if (searchValue) {
                const employee = window.employeeList.find(e => 
                ...
                if (employee) {
                    ...
                    alert('Employee not found. Please search and select from the dropdown.');
                }
            }
        });
    }
