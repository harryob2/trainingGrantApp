...
ADMIN_PASSWORD_HASH = hashlib.sha256("cork4liam".encode()).hexdigest()

class User(UserMixin):
    """User class for Flask-Login."""
    
    def __init__(self, username, dn=None, display_name=None, email=None, first_name=None, last_name=None, groups=None, is_admin=False):
    ...
    def get(username):
        ...
        if username.lower() == ADMIN_EMAIL.lower():
            return User(
        ...
        # For other users, just return a basic User object
...
def init_auth(app):
    ...
    def save_user(dn, username, data, memberships):
        ...
        if required_group:
            logger.info(f"Checking if user {username} is in required group: {required_group}")
            # Flask-LDAP3-Login will pass memberships as a list of group DNs
            if not memberships or required_group not in [group.split(',')[0].split('=')[1] for group in memberships]:
                logger.warning(f"User {username} is not in required group {required_group}")
                return None
...
def authenticate_user(username, password):
    """Authenticate a user with LDAP or admin credentials."""
